...
function traj_youBot_ws()
    ...
    %=============== Graphicial setup====================
    ax = createStage([-6 10 -10 10 -2 12], [55 25]);
...
%     hArmConfig3 = uicontrol('Style', 'checkbox', 'String', 'k_arm3 = -1', 'Position', [120, 5, 100, 20], 'Value', 1);
    ...
    ylabel('\alpha in deg/s^2');
    grid on;
    
    bx=figure('Name','work space');
    set(bx,'units','normalized','outerposition',[0.2 0.2 0.75 0.5]);
    bxPos = axes('OuterPosition', [0 2/3 0.5 1/3], 'XLim', [TA TE]);
    hX = line('XData', TA, 'YData', 0, 'Color', 'r', 'Marker', '.');
    hY = line('XData', TA, 'YData', 0, 'Color', 'g', 'Marker', '.');
    hZ = line('XData', TA, 'YData', 0, 'Color', 'b', 'Marker', '.');
    legend('x','y','z');
    ylabel('position in cm');
    grid on;
    bxAng = axes('OuterPosition', [0.5 2/3 0.5 1/3], 'XLim', [TA TE]);
    hPitch = line('XData', TA, 'YData', 0, 'Color', 'm', 'Marker', '.');
    hRoll = line('XData', TA, 'YData', 0, 'Color', 'k', 'Marker', '.');
    legend('pitch', 'roll');
    ylabel('angle in deg');
    grid on;
    bxPosDot = axes('OuterPosition', [0 1/3 0.5 1/3], 'XLim', [TA TE]);
    hXDot = line('XData', TA, 'YData', 0, 'Color', 'r', 'Marker', '.');
    hYDot = line('XData', TA, 'YData', 0, 'Color', 'g', 'Marker', '.');
    hZDot = line('XData', TA, 'YData', 0, 'Color', 'b', 'Marker', '.');
    legend('x','y','z');
    ylabel('v in cm/s');
    grid on;
    bxAngDot = axes('OuterPosition', [0.5 1/3 0.5 1/3], 'XLim', [TA TE]);
    hPitchDot = line('XData', TA, 'YData', 0, 'Color', 'm', 'Marker', '.');
    hRollDot = line('XData', TA, 'YData', 0, 'Color', 'k', 'Marker', '.');
    legend('pitch', 'roll');
    ylabel('\omega in deg/s');
    grid on;
    bxPos2Dot = axes('OuterPosition', [0 0 0.5 1/3], 'XLim', [TA TE]);
    hX2Dot = line('XData', TA, 'YData', 0, 'Color', 'r', 'Marker', '.');
    hY2Dot = line('XData', TA, 'YData', 0, 'Color', 'g', 'Marker', '.');
    hZ2Dot = line('XData', TA, 'YData', 0, 'Color', 'b', 'Marker', '.');    
    legend('x','y','z');
    ylabel('a in cm/s^2');
    grid on;
    bxAng2Dot = axes('OuterPosition', [0.5 0 0.5 1/3], 'XLim', [TA TE]);
    hPitch2Dot = line('XData', TA, 'YData', 0, 'Color', 'm', 'Marker', '.');
    hRoll2Dot = line('XData', TA, 'YData', 0, 'Color', 'k', 'Marker', '.');
    legend('pitch', 'roll');
    ylabel('\alpha in deg/s^2');
    grid on;
...
%         set(hArmConfig3,'Enable','off');
        ...
        % Format: [t x y z pitch roll vx vy vz omega_pitch omega_roll 
        %          ax ay az alpha_pith alpha_roll theta1 theta2 theta3 theta4 theta5
        %          omega1 omega2 omega3 omega4 omega5 alpha1 alpha2 alpha3 alpha4 alpha5]
        recData = [];
...
%         end
        ...
        % Calculate coefficients for each DOF
        coeff = genTrajWS(PoseA, VelocityA, PoseE, VelocityE);

        %  animate the trajectory
        animate(TA:TDelta:TE,@animStep,TDelta);
        
        disp('Maximum velocity:');
        fprintf('Vx: %9.2f m/s\nVy: %9.2f m/s\nVz: %9.2f m/s\n', max(abs(recData(:, 7:9)))/100 );
        disp('Maximum roll and pitch angular velocity:');
        fprintf('omegaPitch: %9.2f deg/s\nomegaRoll: %9.2f deg/s\nVz: %9.2f m/s\n', max(abs(recData(:, 10:11))));
        disp('Maximum joint angular rate:');        
        fprintf('omega1: %9.2f deg/s\nomega2: %9.2f deg/s\nomega3: %9.2f deg/s\nomega4: %9.2f deg/s\nomega5: %9.2f deg/s\n', max(abs(recData(:, 22:26))) * 180 / pi);
		disp('Maximum joint angular acceleration:');
        fprintf('alpha1: %9.2f deg/s^2\nalpha2: %9.2f deg/s^2\nalpha3: %9.2f deg/s^2\nalpha4: %9.2f deg/s^2\nalpha5: %9.2f deg/s^2\n', max(abs(recData(:, 27:31))) * 180 / pi);
...
%         set(hArmConfig3,'Enable','on');
        ...
        function animStep(t)
            ...
            VarDot = zeros(1,5);
            Var2Dot =zeros(1,5);
            % Pose, velocities and accelerations of TCP
            for i=1:5
                Var(i) = [1, t, t^2, t^3]*coeff(:,i);
                VarDot(i) = [0, 1, 2*t, 3*t^2]*coeff(:,i);			
                Var2Dot(i) = [0, 0, 2, 6*t]*coeff(:,i);
            end
            
            % calculate angles velocities and accelerations of joints with inverse kinematics
            Theta_i = ik_Youbot(Var,k_Arm1,k_Arm3);
            
            % Refresh 3D view
            robot.setJoins(Theta_i);
%             tcp = robot.getTcp();
            
            % velocity and acceleration at the beginn
            if isempty(recData) 
                recData = [t, Var, VarDot, Var2Dot, Theta_i, 0, 0, 0, 0, 0, NaN, NaN, NaN, NaN, NaN];
            else	
				Theta_i = nearestAngle(recData(end, 17:21), Theta_i);

				% numerische Differentiation
				ThetaDot_i = (Theta_i - recData(end, 17:21)) / TDelta;
                
				if size(recData,1) < 2
                    Theta2Dot_i = [NaN NaN NaN NaN NaN]; % calculate no acceleration for first 2 steps
                else
                    Theta2Dot_i = (ThetaDot_i - recData(end, 22:26)) / TDelta;
                end              
                
                % record data for this step
				recData = [recData; t, Var, VarDot, Var2Dot, Theta_i, ThetaDot_i, Theta2Dot_i];
            end
            ...
            arrayfun(@(h, i)set(h, 'XData', recData(:, 1), 'YData', recData(:, i)*180/pi), ...
					 [hTheta1, hTheta2, hTheta3, hTheta4, hTheta5, hThetaDot1, hThetaDot2,...
                     hThetaDot3, hThetaDot4, hThetaDot5, hTheta2Dot1, hTheta2Dot2, hTheta2Dot3,...
                     hTheta2Dot4, hTheta2Dot5], 17:31);
            arrayfun(@(h, i)set(h, 'XData', recData(:, 1), 'YData', recData(:, i) ), ...
					 [hX, hY, hZ,  hXDot, hYDot, hZDot, hX2Dot, hY2Dot, hZ2Dot], [2:4,7:9,12:14]);
            arrayfun(@(h, i)set(h, 'XData', recData(:, 1), 'YData', recData(:, i)), ...
					 [hPitch, hRoll, hPitchDot, hRollDot, hPitch2Dot, hRoll2Dot], [5,6,10,11,15,16]);
    ...
    function A=genTrajWS(startP,startV,endP,endV)
        % Calculate coefficients of the trajectories for each dimension
        A = zeros(4,5);
        B = [1, TA, TA^2, TA^3;...
            0, 1, 2*TA, 3*TA^2;...
            1, TE, TE^2, TE^3;...
        ...
        inverseB = inv(B);
        for i=1:5
           A(:,i) = inverseB*[startP(i);startV(i);endP(i);endV(i)]; 
...
%         end
    ...
    function out = nearestAngle(old, new)		
		delta = new - old;
		out = new - 2 * pi * sign(delta) .* floor((abs(delta) + pi) / (2 * pi));
	end
end
